type STrim<T extends string> = T extends `${infer P}${infer W}`
  ? P extends " "
    ? STrim<W>
    : T
  : T;
type Trim<T extends string> = Reserve<STrim<Reserve<STrim<T>>>>;
type Reserve<T extends string> = T extends `${infer P}${infer W}`
  ? `${Reserve<W>}${P}`
  : "";
type a = Trim<"  a  Brr ">;
...
type Filter<T extends any[], A> = T extends [infer F, ...infer P]
  ? F extends A
    ? [F]
    : [1]
  : [];
